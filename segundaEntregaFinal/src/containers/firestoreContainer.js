...
export default class FirestoreContainer {
  ...
  async addToCart(id, idProduct) {
    ...
    if (!productToAdd) return 'No encontramos el producto a agregar';
  ...
  async deleteProductFromCart(id, idProduct) {
    ...
    if (!cartProducts) return `No encontramos el carrito que buscas...`;
  ...
  async emptyCart(id) {
    ...
    if (!cartToEmpty) return 'No encontramos ese carrito';
